...
build() {
	cd "$srcdir/Google-Drive-CLI"
	local ldflags="-s -w -buildid= -X github.com/dl-alexandre/gdrive/internal/cli.version=${pkgver}"
	go build -trimpath -buildmode=pie -mod=readonly -ldflags "$ldflags" -o gdrive ./cmd/gdrive
}

package() {
	cd "$srcdir/Google-Drive-CLI"
	install -Dm755 gdrive "$pkgdir/usr/bin/gdrive"
